...
void walk_var_dec(ast t)
{
    ...
    if (segment=="static"){
        test7=true;
        write_to_output(space+segment+" "+ type +" "+ name +" ;\n");
    }else{
        write_to_output(space+space+"var "+ type +" "+ name +" ;\n");
    }
    //"var "
    havevardec=true;
}
...
void walk_function(ast t)
{
    ...
    write_to_output(space+"function"+" "+vtype+" "+name+"()"+"\n");
       if(test7==false){
...
void walk_let(ast t)
{
    ...
    if(test7==true){
        write_to_output(space+space+"let ");
        walk_var(var) ;
        write_to_output(" = ");
        walk_expr(expr) ;
        write_to_output(" + 7 ;\n");
...
void walk_if_else(ast t)
{
    ...
    write_to_output(space+space+"if "+"(");
    walk_expr(condition) ;
    write_to_output(")");

    write_to_output("\n"+space+space+"{\n");
    write_to_output(space+space+"}\n");

    write_to_output(space+space+"else");
...
void walk_while(ast t)
{
    ...
    write_to_output(space+space+"while (");
    walk_expr(condition) ;
    write_to_output(")");
    write_to_output("\n"+space+space+"{\n");
...
void walk_return(ast t)
{
    write_to_output(space+space+"return ;");
}
...
void walk_bool(ast t)
{
    ...
    write_to_output(BoolToString(_constant));
}
